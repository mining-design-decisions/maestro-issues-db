...
from .test_util import setup_users_db, restore_dbs, get_auth_header, auth_test_post, auth_test_delete, auth_test_put
...
def setup_db():
    ...
    models_collection.insert_one({
        ...
        'performances': {str(performance_id): {'description': 'performance description'}}
    })
...
def test_update_version_description():
    ...
    client.put(f'/models/{model_id}/versions/{version_id}/description', json=payload, headers=headers)
    model = models_collection.find_one({'_id': model_id})
    assert model['versions'][str(version_id)]['description'] == 'new-description'

    # model not found
    response = client.put(f'/models/{ObjectId()}/versions/{version_id}/description', json=payload, headers=headers)
    assert response.status_code == 404

    # version not found
    response = client.put(f'/models/{model_id}/versions/{ObjectId()}/description', json=payload, headers=headers)
    assert response.status_code == 404
...
def test_post_performance():
    ...
    assert performance_id in models_collection.find_one({'_id': model_id})['performances']
...
def test_update_performance_description():
    ...
    # update description
    payload = {'description': 'new-description'}
    client.put(f'/models/{model_id}/performances/{performance_id}/description', json=payload, headers=headers)
    assert models_collection.find_one(
        {'_id': model_id}
    )['performances'][str(performance_id)]['description'] == 'new-description'

    # model not found
    response = client.put(f'/models/{ObjectId()}/performances/{performance_id}/description',
                          json=payload,
                          headers=headers)
    assert response.status_code == 404

    # version not found
    response = client.put(f'/models/{model_id}/performances/{ObjectId()}/description',
                          json=payload,
                          headers=headers)
    assert response.status_code == 404
